...
export class Main2Component implements OnInit {
  ...
  ngOnInit(): void {
    ...
    function setLayout() {
      ...
      for (let i = 0; i < sceneInfo.length; i++) {
        if (sceneInfo[i].type === 'sticky') {
          ...
          sceneInfo[i].scrollHeight =
            (sceneInfo[i].objs.content as HTMLElement).offsetHeight +
          ...
          if (scrollRatio < values.rect1X[2]['end']) {
            ...
            if (
              ...
              values.canvasCaption_translateY[2]['end'] =
                values.canvasCaption_opacity[2]['end'];
              (objs.canvasCaption as HTMLElement).style.opacity = calcValues(
                values.canvasCaption_opacity,
                currentYOffset
              );
              (objs.canvasCaption as HTMLElement).style.transform = `translate3d(0, ${calcValues(
                values.canvasCaption_translateY,
                currentYOffset
              )}%, 0)`;
            } else {
              (objs.canvasCaption as HTMLElement).style.opacity = `${values.canvasCaption_opacity[0]}`;
            }
          }
    ...
    function loop() {
      ...
      if (delayedYOffset < 1) {
        scrollLoop();
        (sceneInfo[0].objs.canvas as HTMLCanvasElement).style.opacity = '1';
    ...
    window.addEventListener('load', () => {
      ...
      /*     document
        ...
        .addEventListener('transitionend', (e: any) => {
